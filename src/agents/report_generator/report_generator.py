...
class ReportGenerator(BaseAgent):
    ...
    async def _prepare_executor(self):
        ...
        def _get_data(data_id: int):
            ...
            try:
                ...
                if 0 <= data_id < len(collect_data_list):
                    ...
                    self.logger.warning(f"Invalid data_id: {data_id}. Range: 0-{len(collect_data_list)-1}")
                    return None
            except (ValueError, TypeError):
                self.logger.error(f"data_id must be an integer, got {type(data_id)}")
                return None
        ...
        def _get_deepsearch_result(query: str):
            ...
            if not self.tools or not self.tools[0]:
                self.logger.error("DeepSearchAgent not found in tools list")
                return "Error: DeepSearchAgent not available."
            ...
            try:
                ...
                if not output or 'final_result' not in output:
                    self.logger.warning(f"DeepSearch returned empty output for query: {query}")
                    return "No results found from deep search."
                    
                return output['final_result']
            except Exception as e:
                self.logger.error(f"Error during deep search: {e}", exc_info=True)
    ...
    async def _replace_image_path(self, report):
        ...
        for section in report.sections:
            section_new_content = []
            for p_paragraph in section._content:
                # Use a non-greedy regex to match individual @import statements
                match = re.findall(r'@import\s*".*?"', p_paragraph)
                try:
                ...
                if match and len(match) > 0:
                    for img_name in match:
                        ...
                        search_results = await index.search(img_name)
                        if not search_results:
                            self.logger.warning(f"No match found for image placeholder: {img_name}")
                            p_paragraph = p_paragraph.replace(img_name, "")
                            continue
                            
                        most_similar_idx = search_results[0]['id']
            ...
            for p_paragraph in section._content:
                ...
                for match_item in match_list:
                    ...
                    if not search_result:
                        self.logger.warning(f"No reference data found for citation: {match_item}")
                        continue
