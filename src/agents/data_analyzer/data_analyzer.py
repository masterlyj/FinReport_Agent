...
class DataAnalyzer(BaseAgent):
    ...
    async def _handle_max_round(self, conversation_history):
        ...
        # Handle potential markdown wrappers if json_object mode was bypassed or failed
        clean_response = response
        if '```json' in clean_response:
            match = re.search(r'```json([\s\S]*?)```', clean_response)
            if match:
        ...
        # If there's text before/after the JSON, try to extract just the JSON part
        ...
        try:
            report = json_repair.loads(clean_response)
            report_title = report.get("title", "Research Report")
            report_content = report.get("content", response)
