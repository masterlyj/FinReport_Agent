...
class BaseAgent:
    ...
    def _get_api_descriptions(self) -> str:
        desc = 'The usage of function calling: `result = call_tool(tool_name=\'function_name\', **kwargs)`. (you can use custom variable names for the result)\n\n'
        desc += 'Below are the available functions and their descriptions:\n\n'
        for tool in self.tools:
            if issubclass(type(tool), Tool):
                desc += f"- Function: {tool.name}\nDescription: {tool.description}\nArguments: {tool.parameters}\n\nOutput: "
            elif issubclass(type(tool), BaseAgent):
                desc += f"- Function: {tool.AGENT_NAME}\nDescription: {tool.AGENT_DESCRIPTION}\n\n"
        desc += 'The result of each function is a variable, please use `print` to print the result.'
        return desc
